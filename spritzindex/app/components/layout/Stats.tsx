...
const getPrezziDalDB = async (): Promise<BarData[]> => {
  const { data, error } = await supabase
    .from('bars')
    .select('location_provincia, price, type');
    
  if (error || !data || data.length === 0) return [];
  return data as BarData[];
};

const calcolaFrequenzaPrezzi = (datiPrezzi: BarData[]) => {
  const fasce = [
...
const calcolaPrezziMediPerProvincia = (datiGrezzi: BarData[], tipoFiltro?: string) => {
...
const calcolaPrezziMediPerProvinciaETipo = (datiGrezzi: BarData[]) => {
...
export default async function MapOverlay() {
  ...
  return (
    <main className="flex flex-col items-center p-4">
      <div className="grid w-full gap-4 grid-cols-1 md:grid-cols-2 items-start">
        ...
        {/* 1. Box Classifica Prezzi */}
        <div className="w-full font-bold text-black max-w-2xl mx-auto">
          <AspectRatio ratio={1} className="w-full overflow-hidden rounded-3xl bg-white/80 backdrop-blur-md p-6 shadow-xl border border-orange-100 flex flex-col justify-between">
            <div>
        ...
        {/* 2. Matrice Prezzi Medi per Provincia e Tipo Spritz */}
        <div className="w-full font-bold text-black max-w-2xl mx-auto">
          <AspectRatio ratio={4/4} className="w-full overflow-hidden rounded-3xl bg-white/80 backdrop-blur-md p-6 shadow-xl border border-orange-100 flex flex-col justify-between">
        ...
        {/* 3. Box statistiche con Istogramma delle Frequenze */}
        <div className="w-full font-bold text-black max-w-2xl mx-auto">
          <AspectRatio ratio={4/4} className="w-full overflow-hidden rounded-3xl bg-white/80 backdrop-blur-md p-6 shadow-xl border border-orange-100 flex flex-col justify-between">
            <div className="w-full font-bold text-black">
                    ...
                    <div key={fascia.etichetta} className="space-y-1">
                      ...
                      <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden p-0.5 border border-slate-200/60">
                        <div 
                          ...
                          style={{ width: `${Math.max(fascia.percentuale, 4)}%` }} 
                        />
                      </div>
                    </div>
                  ))}
              ...
              <div className="text-center text-xs text-slate-400 pt-2 border-t mt-4">
                Statistiche elaborate sui dati inseriti nel DataBase.
              </div>
            </div>
          </AspectRatio>
        </div>  
